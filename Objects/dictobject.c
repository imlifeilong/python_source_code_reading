...
*/
/* 
该发行版包括一个单独的文件，Objects/dictnotes.txt，描述对字典设计和优化的探索。
它涵盖了典型的字典使用模式、参数调整字典，以及一些可能的优化想法。
*/
...
or DKIX_DUMMY(-2).
dk_indices 是实际的哈希表。 它保存条目中的索引，或 DKIX_EMPTY(-1)或 DKIX_DUMMY(-2)。
Size of indices is dk_size.  Type of each index in indices is vary on dk_size:
索引的大小是 dk_size。 Indexs 中每个索引的类型根据 dk_size 的不同而变化：
...
dk_entries 是 PyDictKeyEntry 的数组。 它的大小是 USABLE_FRACTION(dk_size)。
...
dk_size == 256.
注意：由于 DKIX_EMPTY 和 DKIX_DUMMY 使用负值，因此类型dk_indices 条目是有符号整数，
int16 用于表dk_size==256。
*/


/*
The DictObject can be in one of two forms.
DictObject 可以采用两种形式之一。
...
Either:
  ...
  组合表：
    ...
    值存储在 PyDictKeysObject 的 me_value 字段中。
Or:
  A split table:
  分割表：
    ma_values != NULL, dk_refcnt >= 1
...
1. Unused.  index == DKIX_EMPTY
   ...
   不使用：现在不持有活动的（键，值）对，并且从未持有过。
   未使用的可以在插入键时转换为活动状态。 这是每个插槽的初始状态。

2. Active.  index >= 0, me_key != NULL and me_value != NULL
   Holds an active (key, value) pair.  Active can transition to Dummy or
   Pending upon key deletion (for combined and split tables respectively).
   This is the only case in which me_value != NULL.
   活动的：保存一个活动的（键，值）对。活动可以在键删除时转换为虚拟或待定
   （分别针对组合表和拆分表）。这是 me_value != NULL 的唯一情况。
...
3. Dummy.  index == DKIX_DUMMY  (combined only)
   ...
   虚拟的：以前持有一个活动的（键，值）对，但已被删除，并且活动对尚未覆盖该插槽。插入钥匙后，
   虚拟设备可以转换为活动状态。虚拟时隙不能再次变为未使用状态，
   否则发生冲突时的探测序列将无法知道它们曾经处于活动状态。
...
组合表很简单。 由于 dk_entries 大多数情况下只是追加，因此我们只需迭代 dk_entries 即可获得插入顺序。

One exception is .popitem().  It removes last item in dk_entries and decrement
dk_nentries to achieve amortized O(1).  Since there are DKIX_DUMMY remains in
dk_indices, we can't increment dk_usable even though dk_nentries is
decremented.
一个例外是 .popitem()。 它删除 dk_entries 中的最后一项并减少 dk_nentries 以实现开销为 O(1)。
由于 dk_indices 中还存在 DKIX_DUMMY，因此即使 dk_nentries 减少，我们也无法增加 dk_usable。
...
在拆分表中，仅允许 dk_entries[ix] 插入待处理条目，当 ix == mp->ma_used的时候。
插入其他索引并删除项目会导致将字典转换为组合表。
*/
